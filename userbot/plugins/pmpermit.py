...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "c"
USER_BOT_WARN_ZERO = "`U think that U can spam me??` **Fuck off!** "
USER_BOT_NO_WARN = ("[──▄█▀█▄─────────██ \n▄████████▄───▄▀█▄▄▄▄ \n██▀▼▼▼▼▼─▄▀──█▄▄ \n█████▄▲▲▲─▄▄▄▀───▀▄ \n██████▀▀▀▀─▀────────▀▀](tg://user?id=123456789)\n\n"
                    "`Hii,`"
                    f"You are here now, in my inbox.\n\n"
                    "PLS wait for me to reply.`\n")
    ...
    async def approve_p_m(event):
        ...
        if event.is_private:
            if not pmpermit_sql.is_approved(chat.id):
                ...
                await event.edit("Approved you [{}](tg://user?id={})".format(firstname, chat.id))
